...
void print_help(char *argv[])
{
    std::cout << "Usage: " << argv[0] << " [OPTION]... INPUT_FILE\n"
              ...
              << "                       default: Integrated_<input file>.dat\n"
              << "-h, --help             print help\n";
    exit(1);
}
...
void process_args(int argc, char *argv[], std::string &input_filename)
{
    ...
    if (Options::output_filename.length() == 0)
    {
        Options::output_filename = input_path.parent_path().string() +
                                   "/Integrated_" + input_path.stem().string();
    }
}
...
int main(int argc, char *argv[])
{
    ...
    TimeData::time_multidata_t integrated_data;
    time_data.integrate_all_time(integrated_data, data);
    ...
    for (int istep = 0; istep < num_steps; ++istep)
    {
        ...
        std::vector<double> data_for_writing(data.size());
        for (int idataset = 0; idataset < data_for_writing.size(); ++idataset)
        {
            data_for_writing[idataset] = integrated_data[idataset][istep];
        }
