      ...
      m_coords_epsilon(std::pow(10.0, -a_coords_precision))
{
    ...
    if (m_rank == 0)
    {
        ...
        {
            std::cerr << "SmallDataIO: mode not supported" << std::endl;
            exit(1);
        }
        m_file.open(m_filename, file_openmode);
        if (!m_file)
        {
            std::cerr << "SmallDataIO::error opening file for writing"
                      << std::endl;
            exit(1);
        }
    }
                                          ...
                                          int a_filename_steps_width)
{
    ...
    {
        std::cerr << "SmallDataIO: a_filename_steps_width too small "
                  << "for step number" << std::endl;
        exit(1);
    }
